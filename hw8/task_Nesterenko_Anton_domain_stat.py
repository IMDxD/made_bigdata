...
from pyspark.sql.functions import col, split, approx_count_distinct, count
...
data = data.select(
    col("data").getItem(1).alias("UID"), col("data").getItem(2).alias("URL"),
)

data = data.selectExpr("UID", 'parse_url(URL, "HOST") as domain')

domain_cnt = data.groupBy("domain").agg(
    count("UID").alias("view"), approx_count_distinct(col("UID")).alias("unique")
)

domain_cnt = domain_cnt.sort("view", ascending=False).limit(10)

query = (
    domain_cnt.writeStream.outputMode("complete")
    .format("console")
